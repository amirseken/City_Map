...
#define MWIDTH			125 /* Width of menu window */
#define MENU_FONT_SIZE  9  /* Font for menus and dialog boxes. */
#define T_AREA_HEIGHT	30  /* Height of text window */
...
struct t_x11_state {
    Display *display;
    int screen_num;
    Window toplevel, menu, textarea, search;
    GC gc_normal, gc_xor, gc_menus, current_gc;
    XftDraw *toplevel_draw, *menu_draw, *textarea_draw, *search_draw;
...
static const vector<t_color> predef_colors = {
    ...
    t_color(226, 223, 218) // custom color NUM_COLOR
};
...
static void build_search_box(void);
static void searchtext(XftDraw* draw, const char *text);
...
void create_button(const char *prev_button_text, const char *button_text,
    ...
    button_state.button[i].width = 112;
...
build_default_menu(void) {
    ...
    const int NUM_ARROW_BUTTONS = 4, NUM_STANDARD_BUTTONS = 9, SEPARATOR_BUTTON_INDEX = 8;
            ...
            case KeyPress:
                ...
                if (gl_state.get_keypress_input) {
                    ...
                    if(keyb_buffer[0] == 8){
                        ...
                        Search_Text += keyb_buffer;
                    }
                    XClearWindow(x11_state.display, x11_state.search);
                    searchtext(x11_state.search_draw, Search_Text.c_str());
                }
...
/* Creates a small window at the bottom of the graphics area for text messages */
static void x11_build_textarea(void) {
    XSetWindowAttributes menu_attributes;
    unsigned long valuemask;
...
static void build_search_box(void) {
    x11_state.search = XCreateSimpleWindow(x11_state.display, 
                                            x11_state.toplevel, 
                                            30, 
                                            30, 
                                            500,
    ...
    x11_state.search_draw = XftDrawCreate(x11_state.display,
                                            x11_state.search,
                                            x11_state.visual_info.visual,
                                            x11_state.colormap_to_use);
    ...
    valuemask = CWWinGravity | CWEventMask | CWDontPropagate;
    XChangeWindowAttributes(x11_state.display, x11_state.search, valuemask, &menu_attributes);
    XMapWindow(x11_state.display, x11_state.search);
    searchtext(x11_state.search_draw, "Search...");
}
